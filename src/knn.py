...
class KNN(BaseClassifier):
    """
    A class used to represents a trainable, binary k-nearest neighbor classifier capable of making predictions on
    ...
    def train(self, dataset_url, sep=',', name=None, model_out_dir=None, report_out_dir=None, verbose=0):
        """Trains a new k-nearest neighbor classifier.
        ...
        # Step 6: Find best estimator using cross-validation
        hyperparams = [
            {
                'algorithm': ['auto'],
                'n_neighbors': [3, 5, 7],
        ...
        clf = GridSearchCV(KNeighborsClassifier(), hyperparams, verbose=verbose)
        clf.fit(x_train_scaled, y_train)
        # Step 7: Save classification report and model
        ...
        if report_out_dir is not None:
            self.__dump_classification_report__(
                report, mk_filepath(report_out_dir, get('knn', name), 'report', dataset_url, '.txt'))
        if model_out_dir is not None:
            self.__dump_model__(
                clf, mk_filepath(model_out_dir, get('knn', name), 'clf', dataset_url, '.pkl'))
            self.__dump_scaler__(
                scaler, mk_filepath(model_out_dir, get('knn', name), 'scaler', dataset_url, '.pkl'))
        self.clf = clf
        self.scaler = scaler
