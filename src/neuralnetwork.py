...
class NeuralNetwork(BaseClassifier):
    """
    A class used to represents a trainable, binary neural-network classifier capable of making predictions on
    ...
    def train(self, dataset_url, sep=',', name=None, model_out_dir=None, report_out_dir=None, verbose=0):
        """Trains a new neural-network classifier.
        ...
        clf = GridSearchCV(MLPClassifier(), hyperparams, verbose=verbose)
        ...
        if report_out_dir is not None:
            self.__dump_classification_report__(
                report, mk_filepath(report_out_dir, get('nn', name), 'report', dataset_url, '.txt'))
        if model_out_dir is not None:
            self.__dump_model__(
                clf, mk_filepath(model_out_dir, get('nn', name), 'clf', dataset_url, '.pkl'))
            self.__dump_scaler__(
                scaler, mk_filepath(model_out_dir, get('nn', name), 'scaler', dataset_url, '.pkl'))
        self.clf = clf
        self.scaler = scaler
